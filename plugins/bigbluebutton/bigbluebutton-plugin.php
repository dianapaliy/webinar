...
function bigbluebutton_init_database(){
    ...
    $sql = "INSERT INTO " . $table_name . " (meetingID, meetingName, meetingVersion, attendeePW, moderatorPW)
    VALUES ('".bigbluebutton_generateToken()."','asd11 meeting', '".time()."', 'aa11sdp', 'mas11dp');";
    dbDelta($sql);
...
function bigbluebutton_create_meetings() {
    ...
    if( isset($_POST['SubmitCreate']) && $_POST['SubmitCreate'] == 'Create' ) {
        ...
        $meetingDateTime = htmlspecialchars(stripcslashes($_POST[ 'meetingDateTime' ]));
        ...
        if($meetingName == '' || $attendeePW == '' || $moderatorPW == ''){
            ...
            if(!$alreadyExists){
                $rows_affected = $wpdb->insert( $table_name, array( 'meetingID' => $meetingID, 'meetingName' => $meetingName, 'meetingVersion' => $meetingVersion, 'attendeePW' => $attendeePW, 'moderatorPW' => $moderatorPW, 'waitForModerator' => $waitForModerator? 1: 0, 'recorded' => $recorded? 1: 0, 'meetingDateTime' => $meetingDateTime) );
    ...
    <p>Date of meeting: <input type="datetime-local" name="meetingDateTime" id="meetingDateTime" value=""></p>
...
function bigbluebutton_list_meetings() {
    ...
    if( isset($_POST['SubmitList']) ) { //Creates then joins the meeting. If any problems occur the error is displayed
        ...
        if( $found ){
            ...
            //---------------------------------------------------Update-------------------------------------------------
//            else if($_POST['SubmitList'] == 'Edit' ) { //Obtains the meeting information of the meeting that is going to be update
//
//                //Calls endMeeting on the bigbluebutton server
//                $response = BigBlueButton::endMeeting($found->meetingID, $found->moderatorPW, $url_val, $salt_val );
//
//                //Analyzes the bigbluebutton server's response
//                if(!$response){//If the server is unreachable, then prompts the user of the necessary action
//                    $out .= '<div class="updated"><p><strong>Unable to update the meeting. Please check the url of the bigbluebutton server AND check to see if the bigbluebutton server is running.</strong></p></div>';
//                }
//                else if( $response['returncode'] != 'SUCCESS' && $response['messageKey'] != 'notFound' ) { //If the meeting was unable to be updated due to an error
//                    if($response['messageKey'] == 'checksumError'){
//                        $out .= '<div class="updated"><p><strong>A checksum error occured. Make sure you entered the correct salt.</strong></p></div>';
//                    }
//                    else{
...
//                }
//                else { //The meeting was terminated
//                    $sql = "UPDATE ".$table_name." WHERE meetingID = %s";
//                    $wpdb->query(
//                        $wpdb->prepare($sql, $meetingID)
//                    );
//
...
//            }
    ...
    foreach ($listOfMeetings as $meeting) {
        ...
        if(!$info){//If the server is unreachable, then prompts the user of the necessary action
            ...
            $out .= '
            <form name="form1" method="post" action="" class="meeting-table">
            <input type="hidden" name="meetingID" value="'.$meeting->meetingID.'">
            <tr>
            <td date-type="title"><span>'.$meeting->meetingName.'</span></td>
            <td date-type="id">'.$meeting->meetingID.'</td>
            <td date-type="atpw">'.$meeting->attendeePW.'</td>
            <td date-type="mpw">'.$meeting->moderatorPW.'</td>
            <td date-type="wait">'.($meeting->waitForModerator? 'Yes': 'No').'</td>
            <td date-type="rec">'.($meeting->recorded? 'Yes': 'No').'</td>
            <td  date-type="date">'.$meeting->meetingDateTime.'</td>
            ...
            $out .= '
            <form name="form1" method="post" action="" id="meeting-table2">
            <input type="hidden" name="meetingID" value="'.$meeting->meetingID.'">
            <tr>
            <td date-type="title">'.$meeting->meetingName.'</td>
            <td date-type="id">'.$meeting->meetingID.'</td>
            <td date-type="atpw">'.$meeting->attendeePW.'</td>
            <td date-type="mpw">'.$meeting->moderatorPW.'</td>
            <td date-type="wait">'.($meeting->waitForModerator? 'Yes': 'No').'</td>
...
function bigbluebutton_print_table_header(){
    ...
    <table class="stats" cellspacing="5">
      <tr>
        ...
        <th class="hedextra" colspan="1">Date and Time</td>
